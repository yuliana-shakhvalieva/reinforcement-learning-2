...
class Agent:
    ...
    def train_step(self, batch):
        ...
        action_values = self.model(batch.walls, batch.preys).gather(1, batch.actions.unsqueeze(1).type('torch.LongTensor').to(device)
                                            ).to(device)
        ...
        with torch.no_grad():
            next_state_values[non_final_mask] = torch.max(
                self.target_model(non_final_next_walls, non_final_next_preys)
                , axis=1)[0]
    ...
    def act(self, walls, preys, target=False):
        ...
        if not target:
            actions = self.model(walls, preys)
        else:
            actions = self.target_model(walls, preys)
